...
// import 'package:growon/custom-widgets/custom-date-widget.dart';
import 'package:intl/date_symbol_data_local.dart';
import 'package:intl/intl.dart';
import 'package:rxdart/rxdart.dart';

import '/view/schedule_class/custom-date-widget.dart';
...
class DatePicker extends StatefulWidget {
  ...
  final String locale;
  final List<DateTime> allClassDates;
  final Function(DateTime selectedDate) onMonthYearChange;
  final bool deactivateFuture;
  final bool restrictPrevious;
  ...
  DatePicker(
    ...
    this.width = 60,
    this.height = 80,
    this.deactivateFuture = false,
    this.restrictPrevious = false,
    this.controller,
    @required this.allClassDates,
...
class _DatePickerState extends State<DatePicker> {
  ...
  final _streamMonthController = BehaviorSubject<DateTime>();

  @override
  void dispose() {
    _streamMonthController.close();
    super.dispose();
  }
        ...
        children: [
                  ...
                  return InkWell(
                    onTap: () async {
                      var date = await showDatePicker(
                        context: context,
                        initialDate: widget.initialSelectedDate != null
                            ? widget.initialSelectedDate
                            : DateTime.now(),
                        firstDate: widget.restrictPrevious
                            ? DateTime.now()
                                .subtract(Duration(days: widget.daysCount - 1))
                            : DateTime(2020),
                        lastDate: widget.deactivateFuture
                            ? DateTime.now()
                            : DateTime.now().add(Duration(days: 365)),
                      );
                      if (widget.onMonthYearChange != null) {
                        widget.onMonthYearChange(date);
                      }
                    },
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            DateFormat("MMMM")
                                .format(widget.initialSelectedDate)
                                .toUpperCase(), // Month
          ...
          Expanded(
            ...
            itemBuilder: (context, index) {
              ...
              _streamMonthController.sink.add(date);
...
//            print("Inside Inactive dates.");
              ...
              bool isSelected = _currentDate != null
                  ...
                  : false;
              // widget.allClassDates.forEach((element) {
              //   log(element.toIso8601String());
              // });
              // Return the Date Widget
              return DateWidget(
                isClassToday: widget.allClassDates.where((element) {
